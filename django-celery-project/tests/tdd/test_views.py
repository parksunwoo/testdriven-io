...
def test_celery_task(client, db, monkeypatch, django_capture_on_commit_callbacks):
    ...
    mock_generate_avatar_thumnail_delay = mock.MagicMock(name="generate_avatar_thumbnail")
    monkeypatch.setattr(tasks.generate_avatar_thumbnail, "delay", mock_generate_avatar_thumnail_delay)
    ...
    member = Member.objects.get(username=fake_member.username)
    assert member.pk
    mock_generate_avatar_thumnail_delay.assert_called_once_with(
        member.pk
    )